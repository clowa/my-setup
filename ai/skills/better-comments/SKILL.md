...
Write comments that earn their place. Guiding principle: **a comment should explain _why_, not restate _what_ the code already says** — and the best comment is often the one you avoided by making the code clearer.
...
2. **Does the comment add something the code cannot?** Intent, a business/external constraint, a warning, public-API docs → keep it and make it a _good_ comment.
...
- **Why, not what.** The code shows _what_; a comment exists to explain _why_.
